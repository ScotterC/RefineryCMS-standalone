  ...
  class Refinery::Admin::ResourcesController < Refinery::Admin::BaseController
  ...
  protected
    ...
    def paginate_resources(conditions={})
      ...
      @resources = Resource.where(:conditions => conditions).order('created_at DESC').page(@paginate_page_number ||= params[:page]).per(Resource.per_page(from_dialog?))
    end
